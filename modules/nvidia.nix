...
}:
{
  ...
  # Програв linux_latest: кожен мінорний реліз ядра — це лотерея «чи зібрався
  # nvidia-модуль», а на legacy-гілці драйвера патчі приходять із затримкою.
  # Програв linux_zen/xanmod: їхній головний козир — планувальник, а ми і так
  # ставимо scx_lavd у юзерспейсі; лишається лише зайвий ризик.
  #
  # УВАГА ПРО ІМ'Я АТРИБУТА: топ-рівневого `pkgs.linuxPackages_6_12` НЕ ІСНУЄ.
  # Перевірено за pkgs/top-level/all-packages.nix: там є лише
  # linuxPackages (default), linuxPackages_latest, _testing, _zen, _xanmod*.
  # Конкретні версії живуть у linuxKernel.packages.
  # Перевірити доступні: nix eval --raw nixpkgs#linuxKernel.packages --apply builtins.attrNames
